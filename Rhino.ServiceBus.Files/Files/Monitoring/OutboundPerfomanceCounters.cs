...
namespace Rhino.ServiceBus.Files.Monitoring
{
    ...
    public class OutboundPerfomanceCounters : IOutboundPerfomanceCounters
    {
        ...
        public OutboundPerfomanceCounters(string instanceName)
        {
            ...
            try { unsentMessages = new PerformanceCounter(CATEGORY, UNSENT_COUNTER_NAME, instanceName, false); }
            catch (InvalidOperationException ex) { throw new ApplicationException( string.Format(PerformanceCategoryCreator.CANT_CREATE_COUNTER_MSG, CATEGORY, UNSENT_COUNTER_NAME), ex); }
        }
